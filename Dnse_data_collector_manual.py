...
# [TODO 1]
# Không nhúng key hoặc secret cứng vào file. Nếu chưa có đủ thông tin, giữ rỗng và
# chương trình sẽ tự chuyển sang chế độ công khai / fallback.
...
DEFAULT_SYMBOLS = [ "VCB", "BID", "CTG", "TCB", "MBB", "ACB",
    "SSI", "VCI", "HCM", "VND",
    "VIC", "VHM", "KDH", "NLG",
    "HPG", "HSG", "NKG",
    "MWG", "FRT", "PNJ",
    ...
    "VNM", "MSN", "SAB",
    "GAS", "PLX", "POW",
    "GMD", "REE"]   # sửa/thêm mã tùy ý (hoặc dùng --symbols)
...
# ---------- THU THẬP MỖI 15 PHÚT (quyết định của nhóm) ----------
COLLECT_RESOLUTION = "15"          # khung nến mặc định của chế độ --realtime: nến 15 phút
CANDLE_CLOSE_DELAY = 20            # giây chờ sau khi nến đóng rồi mới gọi API (để nguồn kịp cập nhật nến cuối)
WARMUP_DAYS = 7                    # số ngày (lịch) tải lùi mỗi lần: đủ nến khởi động cho EMA50, RSI14, ATR14, MA20 volume
TOKEN_REFRESH_SECONDS = 4 * 3600   # lấy lại token DNSE định kỳ khi chương trình chạy liên tục nhiều giờ
# Giả định (CHƯA XÁC MINH): mốc thời gian 't' của API là GIỜ BẮT ĐẦU của nến (nến 09:00 gồm 09:00-09:15).
# Nếu DNSE trả giờ KẾT THÚC nến thì đổi thành False.
BAR_TIME_IS_START = True
...
def build_auth_payload() -> dict:
    """Tạo payload chỉ với API Key."""
    payload = {"apiKey": DNSE_API_KEY}
    if DNSE_SECRET_KEY:
        payload["secretKey"] = DNSE_SECRET_KEY
    return payload


def get_dnse_access_token() -> str:
    """Xác thực DNSE chỉ bằng API Key."""
    if not DNSE_API_KEY:
        log.info("Chưa gắn API Key DNSE -> chuyển sang chế độ dữ liệu công khai.")
        return ""
    try:
        ...
        log.info("Đã xác thực DNSE thành công bằng API Key.")
        return token
...
def drop_unclosed_candles(df: pd.DataFrame, resolution: str, now: datetime | None = None) -> pd.DataFrame:
    """Bỏ nến CHƯA ĐÓNG (khung trong ngày). Tín hiệu chỉ được tính trên nến đã đóng để tránh nhìn trước tương lai.
    df['time'] là giờ VN không kèm múi giờ; now (nếu truyền) phải là datetime giờ VN."""
    if resolution in ("1D", "1W"):
        return df
    now_vn = (now or datetime.now(VN_TZ)).astimezone(VN_TZ).replace(tzinfo=None)
    end_time = df["time"] + timedelta(seconds=BAR_SECONDS[resolution]) if BAR_TIME_IS_START else df["time"]
    return df[end_time <= now_vn].reset_index(drop=True)


def _finish_prices(
    df: pd.DataFrame, resolution: str, src: str, errors: list[str], closed_only: bool
) -> tuple[pd.DataFrame | None, str | None, list[str]]:
    """Bước cuối của get_prices: bỏ nến chưa đóng; nếu chưa có nến đóng nào thì báo rõ lý do."""
    if closed_only:
        df = drop_unclosed_candles(df, resolution)
    if df.empty:
        return None, src, errors + [f"{src}: chưa có nến {resolution} nào đã đóng"]
    return df, src, errors


def get_prices(
    symbol: str, start: date, end: date, resolution: str,
    token: str = "", source: str = "auto", demo: bool = False, closed_only: bool = True,
) -> tuple[pd.DataFrame | None, str | None, list[str]]:
    ...
    if demo:
        ...
        df = standardize_ohlcv(raw, symbol, resolution, "DEMO", time_is_unix=True)
        return _finish_prices(df, resolution, "DEMO", [], closed_only)
    ...
    for src in order:
        try:
            if src == "DNSE":
                df = fetch_prices_dnse(symbol, start, end, resolution, token)
            else:
                df = fetch_prices_vnstock(symbol, start, end, resolution)
            return _finish_prices(df, resolution, src, errors, closed_only)
...
METRICS = [
    "roe", "roa", "eps", "pe", "pb", "net_margin", "debt_to_equity", "profit_after_tax"
]
METRIC_DIGITS = {
    "roe": 4,
    "roa": 4,
...
METRIC_ALIASES = {
    ...
    "profit_after_tax": (
        "aftertaxprofit", "aftertaxnetprofit", "netprofitaftertax", "profitaftertax",
        "netincome", "netincomeaftertax", "profitaftertaxvn", "loinhuansauthue",
        "loinhuan_sauthue", "loinhuansauthuevnd", "earningsaftertax", "profitaftertaxvnd",
        "profitaftertaxvnd", "netincomeaftertaxvnd", "incomeaftertax"
    ),
}
...
def _tidy_from_item_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    vnstock năm thường trả về nhiều cột kỳ lặp lại / không unique (ví dụ 2018, 2018, ...)
    nên cần làm sạch tên cột kỳ trước khi gắn vào DataFrame cuối cùng."""
    flat = df.copy()
    flat.columns = [str(c) for c in flat.columns]

    meta = [c for c in flat.columns if _norm(c) in {"item", "itemen", "itemid"}]
    if not meta:
        return pd.DataFrame({"period": [], **{m: [] for m in METRICS}})

    period_cols = [c for c in flat.columns if c not in meta]
    period_cols = list(dict.fromkeys(period_cols))

    row_keys = []
    for _, row in flat.iterrows():
        row_keys.append({_norm(row[c]) for c in meta if pd.notna(row[c])})
    ...
    for metric, aliases in METRIC_ALIASES.items():
        ...
        for alias in aliases:
            for idx, keys in enumerate(row_keys):
                if alias in keys:
                    values = pd.to_numeric(flat.iloc[idx][period_cols], errors="coerce").to_numpy()
                    if len(values) == len(tidy):
                        tidy[metric] = values
                    elif len(values) > 0:
                        tidy[metric] = values[: len(tidy)]
                    found = True
                    break
            if found:
                break
...
def load_manual_metrics_csv(csv_path: str) -> pd.DataFrame:
    """Đọc lịch sử chỉ số tài chính từ CSV ticker/year/quarter."""
    df = pd.read_csv(csv_path)
    required = {"ticker", "year_report", "quarter_report"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"CSV chỉ số thiếu cột: {', '.join(sorted(missing))}")

    df = df.copy()
    df["ticker"] = df["ticker"].astype(str).str.strip().str.upper()
    df["year_report"] = pd.to_numeric(df["year_report"], errors="coerce")
    df["quarter_report"] = pd.to_numeric(df["quarter_report"], errors="coerce")
    df = df.dropna(subset=["ticker", "year_report", "quarter_report"])
    df["period"] = (
        df["year_report"].astype(int).astype(str)
        + "-Q"
        + df["quarter_report"].astype(int).astype(str)
    )
    return df


def financial_report_from_manual_csv(
    symbol: str, manual_df: pd.DataFrame, report_type: str,
    max_periods: int = MAX_FINANCE_PERIODS,
) -> dict | None:
    """Tạo báo cáo chuẩn từ CSV chỉ số; trả None nếu CSV không có mã."""
    rows = manual_df[manual_df["ticker"] == symbol].copy()
    if report_type == "year":
        rows = rows[rows["quarter_report"] == 4].copy()
        rows["period"] = rows["year_report"].astype(int).astype(str)
    if rows.empty:
        return None

    rows = rows.sort_values(["year_report", "quarter_report"], ascending=False).head(max_periods)
    source_columns = {
        "roe": "roe", "roa": "roa", "pe": "pe", "pb": "pb",
        "net_margin": "after_tax_margin", "debt_to_equity": "debt_to_equity",
    ...
    for _, row in rows.iterrows():
        ...
        for metric in METRICS:
            column = source_columns.get(metric)
            item[metric] = _to_float(row.get(column), METRIC_DIGITS[metric]) if column else None
        history.append(item)
    ...
    result = {
        ...
        "period_time": latest["period"],
        "history": history,
        **{metric: latest.get(metric) for metric in METRICS},
        "source": "MANUAL_METRICS_CSV",
        "error": None,
    }
    result["missing_metrics"] = [metric for metric in METRICS if result[metric] is None]
...
def normalize_financial_ratio(df: pd.DataFrame, symbol: str, report_type: str, max_periods: int = MAX_FINANCE_PERIODS) -> dict:
    ...
    Trả thêm history tối đa 10 kỳ gần nhất để xem dữ liệu lịch sử."""
    if df is None or df.empty:
    ...
    if max_periods and len(tidy) > max_periods:
        tidy = tidy.iloc[:max_periods].reset_index(drop=True)
    ...
    for _, row in tidy.iterrows():
        item = {"period": str(row["period"]) }
        for metric in METRICS:
            item[metric] = _to_float(row[metric], METRIC_DIGITS[metric])
        history.append(item)
...
def _get_finance_client(symbol: str, period: str):
    """Ưu tiên vnfinancial nếu có sẵn; fallback về vnstock để giữ backup.
    Mục tiêu: BCTC ưu tiên lấy từ vnfinancial vì có lịch sử dài hơn, còn vnstock chỉ giữ vai trò dự phòng."""
    candidate_modules = [
        ("vnfinancial", ["Financial", "VnFinancial", "Finance", "Client"]),
        ("vnstock.api.financial", ["Finance"]),
    ]
    for module_name, classes in candidate_modules:
        try:
        ...
        for cls_name in classes:
            ...
            if cls is not None:
                try:
                    ...
                    try:
                        return cls(symbol=symbol, source="VCI", period=period)
                    except TypeError:
                        return cls(symbol=symbol, period=period)

    from vnstock import Vnstock  # patch backup nếu không có package nào phù hợp
    return Vnstock().stock(symbol=symbol, source="VCI").finance


def _call_financial_method(client, method_name: str, report_type: str, **kwargs):
    """Gọi phương thức tài chính tương thích với cả vnfinancial lẫn vnstock."""
    method = getattr(client, method_name)
    try:
        return method(period=report_type, lang="en", dropna=True, get_all=True, limit=10, **kwargs)
    except TypeError:
        try:
            return method(period=report_type, lang="en", dropna=True, get_all=True, **kwargs)
        except TypeError:
            return method(period=report_type, lang="en", dropna=True, **kwargs)
...
def get_financial_report(
    symbol: str, report_type: str = "quarter", demo: bool = False,
    max_periods: int = MAX_FINANCE_PERIODS, manual_df: pd.DataFrame | None = None,
) -> dict:
    """Lấy chỉ số tài chính kỳ mới nhất: 'quarter' (quý) hoặc 'year' (năm).
    Mặc định lấy tối đa 10 kỳ gần nhất nếu source hỗ trợ.
    """
    symbol = validate_symbol(symbol)
    ...
    if manual_df is not None:
        manual_result = financial_report_from_manual_csv(symbol, manual_df, report_type, max_periods)
        if manual_result is not None:
            return manual_result

    if demo:
        result = normalize_financial_ratio(generate_demo_ratio_frame(symbol, report_type), symbol, report_type, max_periods=max_periods)
        result["source"] = "DEMO"
        return result

    try:
        client = _get_finance_client(symbol, report_type)
        df = _call_financial_method(client, "ratio", report_type)
        result = normalize_financial_ratio(df, symbol, report_type, max_periods=max_periods)
    ...
    # EPS hoặc lợi nhuận sau thuế có thể không nằm trong bảng ratio -> thử lấy từ income_statement.
    if result["status"] != "ERROR":
        try:
            df_is = _call_financial_method(client, "income_statement", report_type)
            res_is = normalize_financial_ratio(df_is, symbol, report_type, max_periods=max_periods)
            if res_is["profit_after_tax"] is not None and res_is["period_time"] == result["period_time"]:
                result["profit_after_tax"] = res_is["profit_after_tax"]
                result["missing_metrics"] = [m for m in result["missing_metrics"] if m != "profit_after_tax"]
                result["status"] = "OK" if not result["missing_metrics"] else "PARTIAL"
            if result["eps"] is None and res_is["eps"] is not None and res_is["period_time"] == result["period_time"]:
                result["eps"] = res_is["eps"]
                result["missing_metrics"] = [m for m in result["missing_metrics"] if m != "eps"]
                result["status"] = "OK" if not result["missing_metrics"] else "PARTIAL"
        except Exception as err:  # noqa: BLE001
            log.debug("Không lấy được EPS / lợi nhuận sau thuế từ income_statement (%s): %s", symbol, err)
    return result
...
def open_db(db_path: str) -> sqlite3.Connection:
    ...
    columns = {row[1] for row in conn.execute("PRAGMA table_info(financials)")}
    if "profit_after_tax" not in columns:
        conn.execute("ALTER TABLE financials ADD COLUMN profit_after_tax REAL")
        conn.commit()
    return conn
...
def save_financial(conn: sqlite3.Connection, record: dict) -> bool:
    ...
    conn.execute(
        ...
        (symbol, period_type, period_time, roe, roa, eps, pe, pb,
         net_margin, debt_to_equity, profit_after_tax, status, source, collected_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
...
def run_pipeline(
    ...
    financial_csv: str | None = None,
) -> list[dict]:
    """Thu thập -> làm sạch -> chuẩn hóa -> lưu (SQLite + CSV) cho danh sách mã."""
    os.makedirs(out_dir, exist_ok=True)
    token = "" if demo else get_dnse_access_token()
    manual_df = load_manual_metrics_csv(financial_csv) if financial_csv else None
    ...
    for i, raw_symbol in enumerate(symbols):
        ...
        fin_q = get_financial_report(symbol, "quarter", demo, manual_df=manual_df)
        fin_y = get_financial_report(symbol, "year", demo, manual_df=manual_df)
...
def _is_bar_close_in_session(b: datetime) -> bool:
    """b là thời điểm ĐÓNG nến (giờ VN). Phiên: T2-T6, nến đóng trong (9:00, 11:30] và (13:00, 15:00]."""
    if b.weekday() >= 5:
        return False
...
def next_collection_time(
    now: datetime, resolution: str = COLLECT_RESOLUTION,
    delay: int = CANDLE_CLOSE_DELAY, all_hours: bool = False,
) -> datetime:
    """Thời điểm thu thập kế tiếp = mốc nến đóng (căn theo giờ, vd :00 :15 :30 :45) + delay giây.
    Bỏ qua nghỉ trưa, ngoài giờ, cuối tuần (trừ khi all_hours). Không xử lý ngày lễ."""
    step = BAR_SECONDS[resolution]
    base = (int(now.timestamp()) // step) * step
    for k in range(14 * 86400 // step + 2):          # tìm tối đa 14 ngày tới
        boundary = datetime.fromtimestamp(base + k * step, VN_TZ)
        due = boundary + timedelta(seconds=delay)
        if due > now and (all_hours or _is_bar_close_in_session(boundary)):
            return due
    return now + timedelta(seconds=step)               # dự phòng (gần như không xảy ra)


def _sleep_until(target: datetime) -> None:
    """Ngủ đến thời điểm target (chia nhỏ để Ctrl+C phản hồi nhanh)."""
    while True:
        remaining = (target - datetime.now(VN_TZ)).total_seconds()
        if remaining <= 0:
            return
        time.sleep(min(remaining, 30))


def _count_rows(conn: sqlite3.Connection, symbol: str, resolution: str) -> int:
    return conn.execute("SELECT COUNT(*) FROM prices WHERE symbol=? AND resolution=?",
                        (symbol, resolution)).fetchone()[0]


def export_prices_csv(conn: sqlite3.Connection, resolution: str, path: str) -> int:
    """Xuất toàn bộ giá của 1 khung từ DB ra CSV (mở được bằng Excel). Trả về số dòng."""
    df = pd.read_sql_query(
        "SELECT symbol, resolution, time, open, high, low, close, volume, source FROM prices "
        "WHERE resolution=? ORDER BY symbol, time", conn, params=(resolution,))
    if not df.empty:
...
def _collect_once(
    conn: sqlite3.Connection, symbols: list[str], resolution: str, price_source: str,
    token: str, demo: bool, csv_path: str, pause: float,
) -> None:
    """Một lần thu thập: tải nến (kèm dữ liệu khởi động), bỏ nến chưa đóng, lưu DB, xuất CSV."""
    today = datetime.now(VN_TZ).date()
    start = today - timedelta(days=WARMUP_DAYS)
    for i, raw_symbol in enumerate(symbols):
        symbol = validate_symbol(raw_symbol)
        df, used, errs = get_prices(symbol, start, today, resolution, token, price_source, demo)
        if df is None:
            log.warning("%s: chưa có nến đã đóng / chưa có dữ liệu (%s)", symbol, "; ".join(errs)[:200])
        else:
            before = _count_rows(conn, symbol, resolution)
            save_prices(conn, df)
            added = _count_rows(conn, symbol, resolution) - before
            last = df.iloc[-1]
            print(f"[{datetime.now(VN_TZ):%H:%M:%S}] {symbol:<6} nến đóng {last['time']:%Y-%m-%d %H:%M} | "
                  f"giá đóng {last['close']:>10,.2f} | KL {int(last['volume']):>10,} | "
                  f"+{added} nến mới | nguồn {used}")
        if pause > 0 and i < len(symbols) - 1:
            time.sleep(pause)
    export_prices_csv(conn, resolution, csv_path)


def run_realtime(
    symbols: list[str], resolution: str, price_source: str, db_path: str, out_dir: str,
    interval: int | None, iterations: int | None, demo: bool,
    all_hours: bool = False, pause: float = REQUEST_PAUSE,
) -> None:
    """Chế độ THU THẬP ĐỊNH KỲ (mặc định: nến 15 phút, mỗi 15 phút).

    - Lần đầu: gọi ngay, tải lùi WARMUP_DAYS ngày (dữ liệu khởi động cho EMA50, RSI14, ATR14...).
    - Các lần sau: đợi ĐÚNG mốc nến đóng (09:15, 09:30, ..., 11:30, 13:15, ..., 15:00 giờ VN)
      + CANDLE_CLOSE_DELAY giây; bỏ qua nghỉ trưa, ngoài giờ, cuối tuần (trừ khi --all-hours).
    - Nếu truyền interval (giây): dùng chu kỳ cố định thay cho căn mốc nến.
    - Chỉ lưu NẾN ĐÃ ĐÓNG (tránh nhìn trước tương lai); ghi SQLite và xuất prices_<khung>.csv.
    (API nến không phải luồng từng lệnh; muốn tick thật cần MQTT/WebSocket của DNSE.)"""
    if resolution in ("1D", "1W"):
        raise ValueError("Chế độ thu thập định kỳ chỉ dùng cho khung trong ngày (1, 5, 15, 30, 1H)")
    os.makedirs(out_dir, exist_ok=True)
    have_keys = bool(DNSE_API_KEY and DNSE_SECRET_KEY)
    token = "" if demo else get_dnse_access_token()
    token_time = time.time()
    conn = open_db(db_path)
    csv_path = os.path.join(out_dir, f"prices_{resolution}.csv")
    schedule = (f"chu kỳ cố định {interval}s" if interval else
                f"đúng mốc nến đóng {resolution} (+{CANDLE_CLOSE_DELAY}s)"
                + ("" if all_hours else ", chỉ trong giờ giao dịch"))
    log.info("Bắt đầu thu thập %s | khung %s | %s | nạp lùi %d ngày. Nhấn Ctrl+C để dừng.",
             ", ".join(symbols), resolution, schedule, WARMUP_DAYS)
    n = 0
    try:
        while iterations is None or n < iterations:
            n += 1
            if n == 1 and not demo and not is_market_open():
                log.info("Đang ngoài giờ giao dịch (T2-T6, 9:00-11:30 và 13:00-15:00): lần đầu chỉ có nến đã đóng gần nhất.")
            if have_keys and not demo and time.time() - token_time > TOKEN_REFRESH_SECONDS:
                token, token_time = get_dnse_access_token(), time.time()   # token có thể hết hạn khi chạy lâu
            try:
                _collect_once(conn, symbols, resolution, price_source, token, demo, csv_path, pause)
            except Exception:  # noqa: BLE001 - một chu kỳ lỗi không được làm chết cả vòng lặp
                log.exception("Chu kỳ thu thập #%d bị lỗi, sẽ thử lại ở chu kỳ sau.", n)
            if iterations is not None and n >= iterations:
                break
            if interval:
                time.sleep(interval)
            else:
                due = next_collection_time(datetime.now(VN_TZ), resolution, all_hours=all_hours)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--start", help="Ngày bắt đầu YYYY-MM-DD (mặc định: 180 ngày trước; nến trong ngày: 7 ngày trước)")
    ...
    p.add_argument("--interval", type=int, default=None,
                   help="Giây giữa 2 lần cập nhật. Bỏ trống = tự thu thập đúng mốc nến đóng (mỗi 15 phút với khung 15)")
    p.add_argument("--all-hours", action="store_true",
                   help="Thu thập cả ngoài giờ giao dịch/cuối tuần (mặc định chỉ trong phiên; hữu ích khi chạy thử)")
    ...
    p.add_argument("--financial-csv", help="CSV lịch sử chỉ số tài chính; ưu tiên dùng cho BCTC khi có mã tương ứng")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.start:
            ...
            start = end - timedelta(days=180 if args.resolution in ("1D", "1W") else WARMUP_DAYS)
        if start > end:
    ...
    if args.realtime:
        resolution = args.resolution if args.resolution not in ("1D", "1W") else COLLECT_RESOLUTION
        run_realtime(symbols, resolution, args.price_source, db_path, out_dir, args.interval,
                     args.iterations, args.demo, args.all_hours, args.pause)
        return 0
 
    snapshots = run_pipeline(symbols, start, end, args.resolution, args.price_source,
                             db_path, out_dir, args.demo, args.pause, args.financial_csv)
